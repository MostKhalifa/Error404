...
useEffect(()=>{
 axios.get("/instructor/viewCourseReviews/6393668c6dd0bd88852031af").then(
    function (response) {
      ...
      <Table sx={{ minWidth: 650 }} aria-label="simple table">
        ...
        <TableBody key={j}>
          {r.reviews.map((row,i) => (
            ...
            >
              <TableCell align="right" key={i}>{row.review}</TableCell>
              <TableCell align="right" key={i}>{row.rating}</TableCell>
              <TableCell align="right" key={i}>{row.reviewedBy}</TableCell>
              <TableCell align="right" key={i}>{row.traineeType}</TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
