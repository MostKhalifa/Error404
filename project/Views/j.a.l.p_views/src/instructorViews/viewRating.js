...
function ViewRating({instructorId}) {
    ...
    const[reviewsAvailable,setReviewsAvailable]=useState({errorMessage:"You currently have no reviews",avaliable:false});
    const[reviews,setReviews]=useState([])
    useEffect(()=>{
        axios.get('instructor/getRating/'+instructorId)//6365564d5ccca85b4590cfd7-63653e09c81ff58c1c877e6d
        .then(
            (res)=>
            {
                ...
                reviews.forEach((review,i) => {
                    switch(true){
                        case (review.Rating<=1): oneStar++ ;break;
                        case (review.Rating>1&&review.Rating<=2):twoStar++;break;
                        case(review.Rating>2&&review.Rating<=3):threeStar++;break;
                        case(review.Rating>3&&review.Rating<=4):fourStar++;break;
                        case(review.Rating>4&&review.Rating<=5):fiveStar++;break;
                    }
                    averageRarting+=review.Rating
                });

                setOverAllRating(Math.floor((averageRarting/reviews.length)*10)/10); 
        ...
        .catch(
            (res)=>
            {
                setReviewsAvailable({errorMessage:res.response.data,Avaliable:false})
            }
        )
    },[])
    return ( 
            ...
            <div className='overAllRating'>
                <div className='starArea'>
                    ...
                    <div className='ratingStars'>
                        <Rating sx={{width:"100%",color:"#f4976c"}} name="InstructorRating" value={overAllRating} precision={0.1} readOnly/>
                    </div>
                ...
                <div className='percentageArea'>
                    <div className='percentage'>
                        ...
                        <div className='percentageBackgroundDiv'><div style={{width:(fiveStars*100).toString()+'%',height:"20px",backgroundColor:"#f4976c",borderRadius:"20px"}}></div></div>
                        <label className='percentageLabel'>{fiveStars*100}%</label>
                    </div>
                    <div className='percentage'>
                        <label className='percentageLabel'>3-4 Stars</label>
                        <div className='percentageBackgroundDiv'><div style={{width:(fourStars*100).toString()+'%',height:"20px",backgroundColor:"#f4976c",borderRadius:"20px"}}></div></div>
                        <label className='percentageLabel'>{fourStars*100}%</label>
                    </div>
                    <div className='percentage'>
                        <label className='percentageLabel'>2-3 Stars</label>
                        <div className='percentageBackgroundDiv'><div style={{width:(threeStars*100).toString()+'%',height:"20px",backgroundColor:"#f4976c",borderRadius:"20px"}}></div></div>
                        <label className='percentageLabel'>{threeStars*100}%</label>
                    </div>
                    <div  className='percentage'>
                        <label className='percentageLabel'>1-2 Stars</label>
                        <div className='percentageBackgroundDiv'><div style={{width:(twoStars*100).toString()+'%',height:"20px",backgroundColor:"#f4976c",borderRadius:"20px"}}></div></div>
                        <label className='percentageLabel'>{twoStars*100}%</label>
                    </div>  
                    <div className='percentage'>
                        <label className='percentageLabel'>0-1 Star </label>
                        <div className='percentageBackgroundDiv'><div style={{width:(oneStars*80).toString()+'%',hieght:"20px",backgroundColor:"#f4976c",borderRadius:"20px"}}></div></div>
                ...
                <div className='ReviewsSection'>
                    ...
                    <div className='reviews'> 
                        {!reviewsAvailable.avaliable&&<h1 className='errorMessage'>{reviewsAvailable.errorMessage}</h1>}
                        {
                            reviews.map(
                                (review)=>{
                                    return( 
                                        <div className='singleReview' key={review.ReviewBody+review.Rating}>
                                            <Fragment>
